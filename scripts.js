...
// Update these variables for desktop vs mobile
let isDesktop = window.innerWidth >= 1024;
let gameWidth = isDesktop ? 900 : 600;
let obstacleStartPosition = isDesktop ? 870 : 580;
let characterBaselineY = isDesktop ? 225 : 150;
let obstacleBaselineY = isDesktop ? 240 : 160;
let coinBaselineY = isDesktop ? 170 : 120;

// Listen for window resize to adjust game dimensions
window.addEventListener('resize', () => {
  isDesktop = window.innerWidth >= 1024;
  gameWidth = isDesktop ? 900 : 600;
  obstacleStartPosition = isDesktop ? 870 : 580;
  characterBaselineY = isDesktop ? 225 : 150;
  obstacleBaselineY = isDesktop ? 240 : 160;
  coinBaselineY = isDesktop ? 170 : 120;

  // Update positions if needed
  resetObstaclePosition();
  resetCoin();
  
  // If game is not active, make sure elements are in their starting positions
  if (!gameStarted) {
    if (dino) dino.style.top = characterBaselineY + 'px';
    if (cactus) {
      cactus.style.left = obstacleStartPosition + 'px';
      cactus.style.top = obstacleBaselineY + 'px';
    }
  }
});
...
// Simplified loading tips system - Add to your scripts.js file
function initSimpleLoadingTips() {
  const container = document.getElementById('loadingTipsContainer');
  ...
  // Game tips
  const tips = [
      'Press <span class="tip-highlight">SPACEBAR</span> to jump over obstacles',
      'Collect <span class="tip-highlight">coins</span> to double your score',
      'Each jump creates a <span class="tip-highlight">TX</span> on Monad',
      'Let\'s <span class="tip-highlight">BREAK MONAD</span>!'
  ];
  
  function showTips() {
      if (!container || !tipElement) return;
      
      // Display container
      container.style.display = 'block';
      
      // Set initial tip
      tipElement.innerHTML = tips[0];
...
function repositionCharacterSelection() {
  ...
  async function updateNFTInfo() {
    try {
        // Initialize and show loading tips
...
// Replace or update the mintPurgedNadsNFT function in your scripts.js file
async function mintPurgedNadsNFT() {
  ...
  try {
    ...
    // First, try to extract from Transfer event (standard ERC-721 event)
    if (receipt && receipt.events) {
      // Find the Transfer event 
      const transferEvent = receipt.events.find(e => e.event === 'Transfer');
      
      if (transferEvent && transferEvent.args) {
        // The third argument (args[2]) in a Transfer event is typically the tokenId
        tokenId = transferEvent.args[2].toString();
        console.log("Found tokenId from Transfer event:", tokenId);
      } else {
        // If we can't find a Transfer event, try looking for any event with 3+ args
        // where the last arg could be the token ID
        const potentialEvent = receipt.events.find(e => e.args && e.args.length >= 3);
        if (potentialEvent && potentialEvent.args) {
          // Often the last argument is the token ID
          tokenId = potentialEvent.args[potentialEvent.args.length - 1].toString();
          console.log("Found potential tokenId from event:", tokenId);
        }
      }
    }
    
    // If we still don't have a token ID, try getting it from the transaction logs
    if (!tokenId && receipt && receipt.logs && receipt.logs.length > 0) {
      // Try to find a Transfer event in the logs (typically ERC-721 Transfer)
      ...
      if (transferLog && transferLog.topics && transferLog.topics.length > 3) {
        // The last topic in a Transfer event is typically the token ID
        // We need to convert from hex and remove leading zeros
        tokenId = parseInt(transferLog.topics[3], 16).toString();
        console.log("Found tokenId from logs:", tokenId);
      }
    }
    
    // If we still can't get a token ID, try querying the contract directly
    if (!tokenId) {
      try {
        // Get the latest token index owned by the user
        const balance = await nftContract.balanceOf(userAddress);
        if (balance.toNumber() > 0) {
          // Get the tokenId at index 0 (if they just minted their first token)
          const latestTokenId = await nftContract.tokenOfOwnerByIndex(userAddress, balance.toNumber() - 1);
          tokenId = latestTokenId.toString();
          console.log("Retrieved tokenId by querying contract:", tokenId);
        }
      } catch (e) {
        console.log("Error querying for token ID:", e);
      }
    }
    
    // If still no tokenId, use a fallback
    if (!tokenId) {
      // Try to use the transaction hash as a way to look up details later
      tokenId = "Check your wallet"; 
      console.log("Could not determine tokenId, using fallback");
    }
    ...
    showMintFailureModal(error.message || "Unknown error occurred");
    mintNftBtn.disabled = false;
...
function spawnCoin() {
  ...
  isCoinActive = true;
  coinPosition = obstacleStartPosition;
  
  let minHeight, maxHeight;
  
  if (isDesktop) {
    // Desktop values - keep coins within reachable jump height
    minHeight = characterBaselineY - 110; // Higher position (lower value)
    maxHeight = characterBaselineY - 60;  // Lower position (higher value)
  } else {
    // Mobile values - adjust as needed
    minHeight = characterBaselineY - 70; // Higher position
    maxHeight = characterBaselineY - 30; // Lower position
  }
  
  // Generate random position between min and max heights
...
function resetCoin() {
  ...
  coin.style.left = `${obstacleStartPosition}px`; // Reset position
  coin.style.top = `${coinBaselineY}px`;
...
function setupCollisionDetection() {
  ...
  function checkCollisions() {
    ...
    // Adjust collision hitbox based on desktop or mobile
    const dinoWidth = isDesktop ? 75 : 50;
    const dinoHeight = isDesktop ? 75 : 50;
    const cactusWidth = isDesktop ? 30 : 20;
    
    // Only check cactus (obstacle) collision - coin collision is handled separately
    if (cactusLeft < dinoWidth && cactusLeft > 0 && dinoTop >= (characterBaselineY - 10)) {
      gameOverSequence();
...
function setupGameControls() {
  ...
  const keyHandler = function(event) {
    if (event.code === "Space") {
      ...
      console.log("Spacebar disabled while game over modal is open");
...
if (closeGameOverModal) {
  closeGameOverModal.addEventListener('click', () => {
    isGameOver = false;  // <-- This is the key line to add
    gameOverModal.style.display = 'none';
